...
public:
    ...
    uint32_t find(std::string str_name)
    {
        ...
        for(uint32_t i = 0; i < inode_table.size(); ++i) {
            if (inode_table[i].get_i_name() == str_name 
                && inode_table[i].get_uid() == VFS_working_uid) {
                inode_id = i;
            }
        }
        if(inode_id == FIND_FALSE)
{
    ...
    // 创建文件函数
    int create(std::string str_name, uint32_t size_of_block = 1)
    {
